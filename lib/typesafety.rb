...
module Typesafety
  ...
  STAR= -1
  DEFAULT_VALUE= []
  VALID_CONDITIONS= {:multiplicity=>1, :ordered=>1, :default=>1, :default_return_parameter=>1}
    ...
    class << cls
      ...
      def operation name, *parameters, &method
        ...
        define_method( name) do |*args|
          unless args.length==passedparams.length then raise TypesafetyException.new([adorn_error[{:error=>:number_of_parameters}]])
          end
    ...
    begin
      conditions.each_key{ |c| 
        unless VALID_CONDITIONS.key? c
          add_error[ :condition].merge!( {:error => c})
        end
      }
  ...
  def association src, trg, kind= :association, link_conditions=DEFAULT_ASSOCIATION_END_CONDITIONS
  ...
  def link src_end, src, trg_end, trg
